...
class TPOTEstimator(BaseEstimator):
    def __init__(self,  scorers,
                        ...
                        # objectives we need to calculate but don't wanna record
    ...
    def fit(self, X, y):
        ...
        def nr_objective_function(pipeline_individual,
                                            X,
                                            y,
                                            is_classification=self.classification,
                                            scorers= [],
                                            cv=self.cv_gen,
                                            other_objective_functions=self.non_record_objectives_functions,
                                            memory=self.memory,
                                            cross_val_predict_cv=self.cross_val_predict_cv,
                                            subset_column=self.subset_column,
                                            **kwargs):
        ...
        self.individual_generator_instance = tpot2.individual_representations.graph_pipeline_individual.j_estimator_graph_individual_generator(
        ...
        if not(self.warm_start and self._evolver_instance is not None):
            ...
            self._evolver_instance = self._evolver(   individual_generator=self.individual_generator_instance,
                                            ...
                                            non_record_objectives_functions=[nr_objective_function],
                                            non_record_objective_functions_weights=self.non_record_objective_functions_weights,
